...
import { DepartamentosService, EntityArrayResponseType } from '../service/departamentos.service';
...
export class DepartamentosComponent implements OnInit {
  ...
  protected readonly departamentosService = inject(DepartamentosService);
  protected readonly activatedRoute = inject(ActivatedRoute);
  protected readonly sortService = inject(SortService);
  protected modalService = inject(NgbModal);
  protected ngZone = inject(NgZone);

  trackId = (item: IDepartamentos): number => this.departamentosService.getDepartamentosIdentifier(item);
  ...
  protected refineData(data: IDepartamentos[]): IDepartamentos[] {
    const { predicate, order } = this.sortState();
    return predicate && order ? data.sort(this.sortService.startSort({ predicate, order })) : data;
  }

  protected fillComponentAttributesFromResponseBody(data: IDepartamentos[] | null): IDepartamentos[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.departamentosService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
